...
const Sweb3 = require('../sbercoinweb3');
...
const { getSbercoinRPCAddress, getDefaultSbercoinAddress, getWalletPassphrase, isWalletEncrypted } = require('../../test/utils');

const { assert } = chai;

console.log(`Your Sbercoin.com RPC address is ${getSbercoinRPCAddress()}`);
console.log(`Your Default Sbercoin.com address is ${getDefaultSbercoinAddress()}`);

describe('Sweb3', () => {
  const Sbercoin_ADDRESS = getDefaultSbercoinAddress();
  let sweb3;

  beforeEach(() => {
    sweb3 = new Sweb3(getSbercoinRPCAddress());
  });
  ...
  describe('getHexAddress()', () => {
    it('returns the hex address', async () => {
      const hexDecodedAddress = bs58.decode(Sbercoin_ADDRESS).toString('hex');
      const hexadecimalAddress = await sweb3.getHexAddress(Sbercoin_ADDRESS);
  ...
  describe('fromHexAddress()', () => {
    it('returns the Sbercoin.com address', async () => {
      const SbercoinAddress = await sweb3.fromHexAddress('17e7888aa7412a735f336d2f6d784caefabb6fa3');
      assert.isString(SbercoinAddress);
      assert.lengthOf(SbercoinAddress, 34);
    });
  });
  ...
  describe('validateAddress()', () => {
    it('returns an object validating the address', async () => {
      let res = await sweb3.validateAddress(Sbercoin_ADDRESS);
      assert.isDefined(res);
    ...
    it('returns an array of address groupings', async () => {
      ...
      if (!_.isEmpty(res)) {
        ...
        if (!_.isEmpty(innerArr)) {
          ...
          qAssert.isSbercoinAddress(item[0]);
  ...
  !_.includes(process.argv, '--cleanenv') ? describe.skip : describe('cleanEnv tests', () => {
    describe('getNewAddress()', () => {
      it('returns a new Sbercoin.com address', async () => {
